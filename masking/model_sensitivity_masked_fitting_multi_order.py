...
starspectrumall34 = read_fits_file.read_nirspec_dat(testspec_path_o34,desired_wavelength_units='micron')
    
waverange = [np.amin(starspectrumall34.wavelength.value[:970]), np.amax(starspectrumall34.wavelength.value[:970])]

starspectrum34 = read_fits_file.read_nirspec_dat(testspec_path_o34,desired_wavelength_units='Angstrom',
...
spec_f.extend(starspectrum34.flux.value)
spec_w.extend(starspectrum34.wavelength.value)
spec_u.extend(starspectrum34.uncertainty.value)

print len(spec_f), testspec_path_o34, 

starspectrumall35 = read_fits_file.read_nirspec_dat(testspec_path_o35,desired_wavelength_units='micron')
    
waverange = [np.amin(starspectrumall35.wavelength.value[:970]), np.amax(starspectrumall35.wavelength.value[:970])]

starspectrum35 = read_fits_file.read_nirspec_dat(testspec_path_o35,desired_wavelength_units='Angstrom',
                                                 wave_range=waverange)
starspectrum35.uncertainty = (np.zeros(len(starspectrum35.flux.value))+1.0/np.float(snr))*starspectrum35.flux.unit

spec_f.extend(starspectrum35.flux.value)
spec_w.extend(starspectrum35.wavelength.value)
spec_u.extend(starspectrum35.uncertainty.value)

print len(spec_f), waverange

starspectrumall36 = read_fits_file.read_nirspec_dat(testspec_path_o36,desired_wavelength_units='micron')
    
waverange = [np.amin(starspectrumall36.wavelength.value[:970]), np.amax(starspectrumall36.wavelength.value[:970])]

starspectrum36 = read_fits_file.read_nirspec_dat(testspec_path_o36,desired_wavelength_units='Angstrom',
                                                 wave_range=waverange)
starspectrum36.uncertainty = (np.zeros(len(starspectrum36.flux.value))+1.0/np.float(snr))*starspectrum36.flux.unit

spec_f.extend(starspectrum36.flux.value)
spec_w.extend(starspectrum36.wavelength.value)
spec_u.extend(starspectrum36.uncertainty.value)


print len(spec_f), waverange

plt.plot(spec_w,spec_f)
plt.show()

spec_f = np.array(spec_f)
spec_w = np.array(spec_w)
spec_u = np.array(spec_u)

starspectrum35 = Spectrum1D.from_array(dispersion=spec_f, flux=spec_w, dispersion_unit=u.angstrom, uncertainty=spec_u) #

'''

print "length of spec",len(starspectrum35.flux.value)
...
gc_result_masked_sl.to_hdf("/u/rbentley/metallicity/spectra_fits/masked_fit_results/sensitivity_cut_"+cutoffstr.replace(".","")+"_NGC6791_J19205+3748282_order34-36.h5")
...
plt.plot(w/(unmasked_median_fits['vrad_2']/3e5+1.0),starspectrum35.flux.value-f,label='Data - Model Residuals')
...
plt.title("Order 35 Observed and Model fluxes (No mask)", size=15)
...
fit_results_file.write("Fitting results with S_l < "+str(sys.argv[1])+" removed orders 34-36\n")
...
multipage("/u/ghezgroup/data/metallicity/nirspec/spectra_fits/pdfs/NGC6791_J19205+3748282_order34-36_sl<"+str(sys.argv[1])+"_cut.pdf")
