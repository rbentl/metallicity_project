...
for i in range(len(data_unmasked_residual)):
    if abs(data_unmasked_residual[i]) > 20.:
...
l1b = dataax.plot(mask_sl_w, mask_sl_f, "ro", markersize=2.5)[0]




l4 = modelax2.plot(sl_wavelength, sl_flux, markersize=2.5)[0]

l4b = modelax2.plot(unmasked_wavelength, unmasked_flux,markersize=2.5)[0]


dataax.set_xticks([])

l1c = dataax.plot(sl_wavelength, sl_flux, color='green')[0]
...
l3 = sl_ax.semilogy(sl_wavelength/(gc_result.median['vrad_2']/3e5+1.0), abs_sl_mh, color='0.5')[0]

l3b = sl_ax.axhline(cutoff, color='black',ls=':')
l3c = sl_ax.axhline(-cutoff, color='black',ls=':')

dataax.set_title(starname+" Order "+order+" with $S_{\lambda}$ < "+cutoff_str+" masked fit\nNumber of flagged points:"+str(len(mask_sl_f)))
...
modelax2.set_title(starname+" Order "+order+" with $S_{\lambda}$ < "+cutoff_str+" masked fit\nNumber of flagged points:"+str(len(mask_sl_f)))
...
fig2.legend([l4, l4b, l5, l6, l6b],['Masked Model', 'Data','Masked Model-Data Residuals', 'Unmasked Model-Masked Model Residuals', '$\pm$5% flux'], loc=(0.4,0.02))
...
dataresidualax  = fig2.add_subplot(2,1,1)
modelresidualax  = fig2.add_subplot(2,1,2)

l3 = dataresidualax.plot(sl_wavelength/(gc_result.median['vrad_2']/3e5+1.0), data_unmasked_residual, color='red')[0]
plotlines.oplotlines(angstrom=True,arcturus=True,alpha=0.1,size=6,highlight=['Sc'])
l4 = dataresidualax.axhline(0.05, color='black',ls='--')

l5 = dataresidualax.axhline(-0.05, color='black',ls='--')

dataresidualax.set_xticks([])

l6 = modelresidualax.plot(sl_wavelength/(gc_result.median['vrad_2']/3e5+1.0), data_unmasked_residual, color='blue')[0]

f.legend([l3, l6, l4],['Masked Model-Data Residuals', 'Unmasked Model-Masked Model Residuals', '$\pm$5% flux'], loc='upper right')
'''
...
print gc_result
plt.show()
#f.savefig('sens_masked_'+cutoff_str+'_v2a.png')
#fig2.savefig('sens_masked_'+cutoff_str+'_v2b.png')
