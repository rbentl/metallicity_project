...
slope_mask_flux = []
slope_mask_wavelength = []

for i in range(len(starspectrum35.flux.value)):
    if abs(slopes[i]) < cutoff:
        slope_mask_flux += [starspectrum35.flux.value[i]]
        slope_mask_wavelength += [starspectrum35.wavelength.value[i]]

        

gc_result = MultiNestResult.from_hdf5("/u/rbentley/metallicity/spectra_fits/masked_fit_results/slope_cut_0.015_NGC6791_J19205+3748282_order35.h5")
...
l1b = dataax.plot(slope_mask_wavelength/(gc_result.median['vrad_2']/3e5+1.0), slope_mask_flux, "ro", markersize=2.5)[0]
...
l3 = sl_ax.plot(sl_wavelength/(gc_result.median['vrad_2']/3e5+1.0), slopes, color='0.5')[0]

l3b = sl_ax.axhline(cutoff, color='black',ls=':')
l3c = sl_ax.axhline(-cutoff, color='black',ls=':')

dataax.set_title("NGC6791_J19205+3748282 Order 35 with slope < "+cutoff_str+" masked fit\nNumber of flagged points:"+str(len(slope_mask_flux)))
...
f.legend([l1, l1b, l2, l3, l3b],["Data","Masked Data Points",'Masked Model','Slope', 'Slope = $\pm$'+cutoff_str], loc=(0.4,0.02))
...
modelax2.set_title("NGC6791_J19205+3748282 Order 35 with slope < "+cutoff_str+" masked fit\nNumber of flagged points:"+str(len(slope_mask_flux)))
...
plt.show()
f.savefig('slope_masked_'+cutoff_str+'_v2a.png')
fig2.savefig('slope_masked_'+cutoff_str+'_v2b.png')
